var App = (function ($) {
    ...
    var handleUI = function () {
        ...
        $('body').on('click', '[data-role="unlink"]', function (e) {
            ...
            showConfirmModal('Confirm Action', msg, function () {
                $.post(href, data, function (res) {
                    if (res['status'] == 1) {
                        element.parent().parent().parent().parent().remove();
                        showToast(res['message'] || 'Deleted successfully', 'Success!', 'success');
                        setTimeout(function () {
                        ...
                        showToast(res['message'] || 'Unable to delete the record.', 'Failed!', 'error');
                    }
                }, 'json');
            });
        });
        ...
        $('body').on('change', '[data-role="active-switch"]', function (e) {
            ...
            // Assuming column 1 is the second column (index 1) in the DataTable
            var $statusButton = $this.closest('tr').find('.ticket'); // Find the corresponding status button

            $.post($this.data('href'), data, function (res) {
                res = JSON.parse(res);
                if (res['status'] == 1) {
                    if ($statusButton.hasClass('active')) {
                        $statusButton.html('<i class="fas fa-times-circle text-danger"></i> Disabled');
                        $statusButton.removeClass('active').addClass('disabled');
                    } else {
                        $statusButton.html('<i class="fas fa-check-circle text-success"></i> Active');
                        $statusButton.removeClass('disabled').addClass('active');
                    }
    ...
    var handleTreeView = function (config={}) {
        
        $('[data-role="ui-datatable"]').each(function () {
            _bindDatatable($(this),config);
        });
        $('[data-role="ui-datatable-static"]').each(function () {
            $(this).DataTable({...{
                ...
                'language': {
                    'paginate': {
                        'next': '<i class="bx bx-right-arrow">Next</i>',
                        'previous': '<i class="bx bx-left-arrow">Previous</i>'
                    }
                },
    ...
    var _bindDatatable = function ($table,config={}) {
        var columns = [];
        var order = [];
        ...
        $table.find('thead th').each(function (index) {
            ...
            if (typeof _cname == 'undefined' || _cname == '') {
                ...
                if(_cname == 'id')
                    {
                        var _c = { 'data': _cname, searchable: false};     
                    }
    ...
    var handleFormView = function () {
        $(window).on('load', function () { $('[type="submit"]').prop("disabled", false); });
    ...
    var showAlertModal = function (message, title) {
        ...
        var $modal = $('<div class="modal fade" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">' + options['title'] + '</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><div class="modal-body">' + options['body'] + '</div><div class="modal-footer"><button type="button" class="main-btn secondary-btn btn-hover">' + options['buttonText'] + '</button></div></div></div></div>');
        $modal.modal('show');

        $modal.off('click', '.secondary-btn');
        $modal.on('click', '.secondary-btn', function () {
            $modal.modal('hide');
        });
    }
    ...
    var button_loading = function (action) {

        if (typeof action === 'undefined') {
            action = false;
        }

        var $loader = $('body').find('[type="submit"]');
        if ($loader.length == 1) {
            if (action === 'show' || action == true) {
                $loader.prop('disabled', true);
                $loader.html('<span class="spinner-border me-1" role="status" aria-hidden="true"></span>Processing...');
            } else {
    ...
    return {
        ...
        initTreeView: function (config) {
            handleTreeView(config);
        },
        ...
        button_loading: function (action) {
            button_loading(action);
        },

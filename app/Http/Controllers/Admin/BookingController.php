...
class BookingController extends Controller
{
    ...
    public function gettotalbookingList()
    {
        ...
        return DataTables::of($sqlBuilder)
            ...
            ->editColumn('qouted_amount', function ($data) {
                $html = '';
            ...
            ->rawColumns(['booking_status', 'is_paid', 'action', 'qouted_amount', 'category_name'])
            ->make(true);
...
//            'drivers' => 'required',
    ...
    public function index_new(REQUEST $request)
    {
        ...
        $mode = "List";
        $route_back = route('admin.dashboard');
        return view('admin.bookings.new', compact('mode', 'page_heading', 'route_back'));
    }
    ...
    public function create_new_request($id = "")
    {
        ...
        return view("admin.bookings.create_new_request", compact('booking', 'page_heading', 'mode', 'id', 'name', 'cost', 'status', 'route_back', 'categories', 'sizes', 'delivery_type', 'category_id', 'size_id', 'delivery_type_selected', 'customers', 'customer_id', 'cares', 'datamain', 'drivers', 'settings'));
    }
    ...
    public function view_request($id = "")
    {
        ...
        return view("admin.bookings.request_view", compact('booking', 'page_heading', 'mode', 'id', 'name', 'cost', 'status', 'route_back', 'categories', 'sizes', 'delivery_type', 'category_id', 'size_id', 'delivery_type_selected', 'customers', 'customer_id', 'cares', 'drivers'));
    }

    public function view_booking_request($id = "")
    {
        $datamain = [];
        if ($id) {
            $datamain = BookingPickUpOrder::with('customer')->find($id);
        ...
        return view("email_templates.booking", compact('user', 'page_heading', 'mode', 'id', 'name', 'cost', 'status', 'route_back', 'categories', 'sizes', 'delivery_type', 'category_id', 'size_id', 'delivery_type_selected', 'customers', 'customer_id', 'cares', 'datamain', 'drivers'));
    }
    ...
    public function update_existing_booking(Request $request) {
        $booking = [
            'pick_up_location' => $request->pu_location,
            'pick_up_lat' => $request->po_latitude,
            'pick_up_lng' => $request->po_longitude,
            'drop_off_location' => $request->do_location,
            'drop_off_lat' => $request->do_latitude,
            'drop_off_lng' => $request->do_longitude,
            'dial_code' => $request->dial_code,
            'mobile_number' => $request->mobile_number,
        ];

        UserBooking::where('id', $request->id)->update($booking);
    ...
    public function create_new_request_store(Request $request)
    {
        ...
        if (!empty($id)) {
            $dataold = BookingPickUpOrder::find($id);
            $old_delivery_driver = $dataold->delivery_driver;
            $old_pickup_driver = $dataold->pickup_driver;
        ...
        if (!empty($dataold)) {
            ...
            if ($old_pickup_driver != $request->pickup_driver) {
                $driver = $request->pickup_driver;
                $notification_send = 1;
            }

            if ($old_delivery_driver != $request->delivery_driver) {
    ...
    {
        $booking_ids = [];
        if ($request->has('p_driver_id')) {
            $user = User::find($request->p_driver_id);
            if ($user) {
                $exploded_vehicles = explode(',', $user->vehicle_type);
                $types = VehicleType::whereIn('model', $exploded_vehicles)->get();
                $booking_ids = $types->pluck('id')->toArray();
            }
        }
        $bookings = UserBooking::when($request->p_driver_id, function ($q) use ($booking_ids) {
            return $q->whereIn('vehicle_type_id', $booking_ids);
        })->latest()->get();
        $sqlBuilder = [];
        foreach ($bookings as $key => $booking) {
            $sqlBuilder[$key]['id'] = $booking->id;
            $sqlBuilder[$key]['order_number'] = $booking->booking_number;
            $sqlBuilder[$key]['delivery_order_number'] = !empty($booking->accepted_order) ? $booking->accepted_order->id : 'N/A';
            $sqlBuilder[$key]['created_at'] = Carbon::parse($booking->created_at)->format('d M, Y');
            $sqlBuilder[$key]['pickup_driver'] = !empty($booking->accepted_order) ? $booking->accepted_order->vendor->name : 'N/A';
            $sqlBuilder[$key]['cat_name'] = !empty($booking->vehicle) ? $booking->vehicle->category->model : 'N/A';
            $sqlBuilder[$key]['cust_name'] = !empty($booking->customer) ? $booking->customer->name : 'N/A';
            $sqlBuilder[$key]['booking_status'] = $booking->booking_status;
        }

        $data_table = DataTables::of($sqlBuilder)
            ->editColumn('created_at', function ($data) {
                return (new Carbon($data['created_at']))->format('d/m/y h:i A');
            })
            ->editColumn('order_number', function ($data) {
                return $modifiedString = str_replace("P-", "IMOVE-", $data['order_number']);
            })
            ...
            ->addColumn('action', function ($data) use ($request) {
                ...
                if (!empty($request->p_driver_id)) {
                    $html .= '<a class="dropdown-item" href="' . route('driver_booking_requests.create', $data['id']) . '?driver=' . $request->p_driver_id . '"><i class="bx bxs-truck"></i>Make a Bid</a>';
                } else {
                    $html .= '<a class="dropdown-item" href="' . route('driver_booking_requests.list', $data['id']) . '"><i class="bx bxs-truck"></i>Driver Requests</a>';
                }
        ...
        if ($request->type == 2) {
            $data_table->addColumn('qr_code', function ($data) {
                return QrCode::generate($data['delivery_order_number'] ?? $data['order_number']);
            });
            $data_table->editColumn('order_number', function ($data) {

                return $data['delivery_order_number'];
            });
        }
        $data_table = $data_table->rawColumns(['user_status', 'action'])
            ->make(true);
        return $data_table;

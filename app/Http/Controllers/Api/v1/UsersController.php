...
class UsersController extends Controller
{
    ...
    public function my_profile(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data = User::with('driver_detail')->where(['users.id' => $user_id])->first();
            if (!empty($data)) {
        ...
        return response()->json(['status' => $status, 'error' => $errors, 'message' => $message, 'user' => $o_data], 200);
    }
    ...
    public function add_address(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $o_data = UserAdress::get_address_details($address->id);
    ...
    public function update_user_profile(REQUEST $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'driving_license' => 'sometimes|required|mimes:jpeg,png,jpg,gif,pdf',
                'emirates_id_or_passport' => 'sometimes|required|mimes:jpeg,png,jpg,gif,pdf',
                'mulkia' => 'sometimes|required|mimes:jpeg,png,jpg,gif,pdf',
            ]);
            ...
            if ($validator->fails()) {
                ...
                if ($request->file("user_image")) {
                    ...
                    $s3Path = 'imove/users/' . $fileName;
                    $filePath = \Storage::disk('s3')->put($s3Path, file_get_contents($file));
                    $user->profile_image = \Storage::disk('s3')->url($s3Path);
                }
                ...
                if (!empty($user) && $user->role_id == 2) {
                    if ($request->vehicle_types) {
                        $user->vehicle_type = $request->vehicle_types;
                    }
                    ...
                    DriverDetail::updateOrCreate(['user_id' => $user->id], $driving_drivers);
                }

                $userdata = User::with('driver_detail')->where(['id' => $user_id])->first();
                $userdata->vehicle_type = $request->vehicle_types;
                $o_data = convertNumbersToStrings($userdata->toArray());
                $status = "1";
                $message = "Profile updated Successfully.";

                if (($user->dial_code != $request->dial_code || $user->phone != $request->phone) && ($request->phone && $request->dial_code)) {
                    $mobile = $request->dial_code . $request->phone;
                    $otp = (string)get_otp(); //rand(1000, 9999);;;
            ...
            return response()->json(['status' => $status, 'message' => $message, 'errors' => $errors, 'user' => $o_data]);
    ...
    public function change_password(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (Hash::check($request->old_password, $user->password)) {
                ...
                Artisan::call('send:send_changepassword_mail', ['user_id' => $user->id]);
            } else {

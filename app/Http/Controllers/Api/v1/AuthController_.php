...
class AuthController extends Controller
{
    ...
    public function email_login(Request $request)
    {
        ...
        $user = User::whereRaw("LOWER(email) = '$lemail'")->where(['role_id' => 3])->first();
        if ($user != null) {
    ...
    public function signup(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // Return JSON response with the first error message in the 'message' key
            return response()->json([
                'status' => "0",
                'message' => $firstErrorMessage,  // Place the first validation error message here
                'error' => (object) $validator->errors(),  // Retain all error messages in the 'error' key as an object
            ], 200);
        }
        
        $user = User::get()->first()->toArray();
        ...
        User::whereRaw("LOWER(email) = '$lemail'")->where('deleted', 0)->where(['role_id' => 2])->delete();

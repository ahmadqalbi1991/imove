...
class UsersController extends Controller
{
    ...
    public function dashboard (Request $request) {
        try {
            ...
            $total_orders = OrderModel::where(['vendor_id' => $user->id])->get();
            $total_pending_orders = $total_orders->where('status', 'pending')->count();
            $completed_orders = $total_orders->where('status', 'completed');
            $exploded_types = explode(',', $user->vehicle_type);
            $types = VehicleType::whereIn('model', $exploded_types)->get();
            $types = $types->pluck('id')->toArray();
            $recent_bookings = UserBooking::whereIn('vehicle_type_id', $types)->where('booking_status', 0)->latest()->get();

            $oData['total_pending_orders'] = (string)$total_pending_orders;
            $oData['total_completed_orders'] = (string)$completed_orders->count();
            $oData['total_earnings'] = (string)$completed_orders->sum('amount');
            $oData['recent_bookings'] = convertNumbersToStrings($recent_bookings->toArray());

            return return_response('1', 200, 'Dashboard fetched', [], $oData);
    ...
    public function my_profile(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($data->count() > 0) {
                ...
                $img = !$o_data->profile_image ? asset("storage/placeholder.png") : asset("storage/user/".$o_data->profile_image);
    ...
    public function update_user_profile(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($file = $request->file("user_image")) {
                $response = image_upload($request, 'user', 'user_image');
                if ($response['status']) {
                    $vendordatils['profile_image'] = $response['link'];
                    User::where('id', $user_id)->update($vendordatils);
                    if($user->role == '2'){
            ...
            $userdata->user_image = !$userdata->profile_image ? asset("storage/placeholder.png") : asset('storage/user/'.$userdata->profile_image);

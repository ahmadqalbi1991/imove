...
class CMS extends Controller
{
    ...
    {
        $datamain = Size::where('status','active')->orderBy('id','desc')->get()->toArray();
       
        return response()->json([
            'status' => (string) 1,
            'message' => 'Data fetched successfully',
            'errors' => [],
            'oData' => convert_all_elements_to_string($datamain),
        ], 200);
    }
    public function carelist(Request $request)
    {
        $datamain = Care::get()->toArray();
    ...
    public function get_page(Request $request)
    {
        ...
        $page_data = Page::where(['id' => $request->page_id])->get();
        if ($page_data->count() > 0) {
            $status = (string) 1;
            $message = "data fetched successfully";
            $o_data = $page_data->first()->toArray();
        }
        return response()->json([
            'status' => $status,
            'message' => $message,
    ...
    public function gethelp(Request $request)
    {
        ...
        $page_data = ContactUsSetting::first()->toArray();
        
        return response()->json([
            'status' => $status,
            'message' => $message,
            'errors' => [],
            'oData' => convert_all_elements_to_string($page_data),
        ], 200);
    }
    function submit_contact_us(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $mailbody =  view("email_templates.contact_us", compact('name', 'email', 'phone', 'msg'));
